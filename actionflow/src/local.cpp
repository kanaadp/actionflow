        ...
        try{	
            ...
            if(array_vec[0].size() == 0){
                ...
                for(int i = 0; i < centers.rows; i++){
                    ...
                    vector_centers.erase(std::find(vector_centers.begin(),vector_centers.end(),closest));
                    ...
                    std::stringstream ss;
                    ss << i+1 <<"," <<  round(x_out * 100.0)/100 <<","<< round(y_out * 100.0)/100 <<",";
                    string pos_string = ss.str();
                    cout << "pos_string: " << pos_string << endl;
                    putText(frame, pos_string, array_vec[i].back(), FONT_HERSHEY_DUPLEX, 1, Scalar(0, 0, 0), 2);
